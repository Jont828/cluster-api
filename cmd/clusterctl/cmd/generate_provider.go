...
func init() {
	...
	generateProviderCmd.Flags().StringVar(&gpo.addonProvider, "add-on", "",
...
func parseProvider() (string, clusterctlv1.ProviderType, error) {
	...
	if gpo.bootstrapProvider != "" {
		if providerName != "" {
			return "", "", errors.New("only one of --core, --bootstrap, --control-plane, --infrastructure, --ipam, --extension, --add-on should be set")
		}
	...
	if providerName == "" {
		return "", "", errors.New("at least one of --core, --bootstrap, --control-plane, --infrastructure, --ipam, --extension, --add-on should be set")
	}
