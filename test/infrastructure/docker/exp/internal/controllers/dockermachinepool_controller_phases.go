...
func (r *DockerMachinePoolReconciler) deleteMachinePoolMachine(ctx context.Context, dockerMachine infrav1.DockerMachine, machinePool expv1.MachinePool) error {
	...
	if machine == nil {
		...
		if mpDeleted := isMachinePoolDeleted(ctx, r.Client, &machinePool); mpDeleted {
			log.V(2).Info("DockerMachine has no owner Machine and MachinePool is deleted, deleting DockerMachine directly", "dockerMachine", dockerMachine.Name, "namespace", dockerMachine.Namespace)
			if err := r.Client.Delete(ctx, &dockerMachine); err != nil {
				return errors.Wrapf(err, "failed to delete DockerMachine %s/%s", dockerMachine.Namespace, dockerMachine.Name)
			}
		} else { // If the MachinePool still exists, then the Machine will be created, so we need to wait for that to happen.
			return errors.Errorf("DockerMachine %s/%s has no owner Machine, will reattempt deletion once parent Machine is present", dockerMachine.Namespace, dockerMachine.Name)
		}
	} else {
