...
func calculateClusterCtlInitArgs(input InitInput) []string {
	...
	if len(input.AddonProviders) > 0 {
		args = append(args, "--add-on", strings.Join(input.AddonProviders, ","))
	}
	return args
}
...
func Upgrade(ctx context.Context, input UpgradeInput) {
	...
	if isCustomUpgrade {
		log.Logf("clusterctl upgrade apply --core %s --bootstrap %s --control-plane %s --infrastructure %s --ipam %s --runtime-extension %s --add-on %s --config %s --kubeconfig %s",
			input.CoreProvider,
